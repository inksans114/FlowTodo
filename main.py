...
from PySide6.QtGui import QColor, QImage, QPainterPath, QRegion
...
class BackendBridge(QObject):
    ...
    def _launch_island(self, tasks, mode, session, options=None):
        ...
        try:
            island_win = DynamicIslandBridge(tasks=tasks, mode=mode, options=self._island_focus_options(options))
            key = id(island_win)
    ...
    def start_group(self, group_json):
        ...
        try:
            ...
            self._launch_island(island_tasks, mode="group", session=session, options=options)
        except Exception as e:
    ...
    def start_project(self, project_json):
        ...
        try:
            ...
            self._launch_island(island_tasks, mode="project", session=session, options=options)
        except Exception as e:
...
class MainWindow(QMainWindow):
    ...
    无边框 + 半透明背景 + 圆角遮罩,内嵌 NoZoomWebView 加载前端 index.html,
    ...
    def __init__(self):
        ...
        self.browser.setAttribute(Qt.WidgetAttribute.WA_OpaquePaintEvent, False)
    ...
    def _update_rounded_mask(self):
        """更新窗口圆角遮罩：最大化时清除遮罩,否则应用 28px 圆角."""
